...
export function SolarOps() {
  ...
  const byRep = Array.isArray(data?.by_rep) ? data.by_rep : []
  ...
  const monthlyChart: ApexCharts.ApexOptions = {
    ...
    xaxis: { categories: byMonth.map(m => m.month) },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <ChartCard title="Monthly Revenue">
                <ReactApexChart
                  ...
                  series={[
                    { name: 'Paid', data: byMonth.map(m => m.paid) },
                    { name: 'Pending', data: byMonth.map(m => m.pending) },
                  ]}
                  type="bar"
                  height={320}
                />
              ...
              <ChartCard title="Rep Performance">
                <ReactApexChart
                  options={{
                    ...repChart,
                    xaxis: { ...repChart.xaxis, categories: byRep.map(r => r.rep) },
                  }}
                  series={[
                    { name: 'Paid', data: byRep.map(r => r.paid) },
                    { name: 'Pending', data: byRep.map(r => r.pending) },
                  ]}
                  type="bar"
                  height={Math.max(200, byRep.length * 50)}
                />
              </ChartCard>
            </div>
          ) : null}
